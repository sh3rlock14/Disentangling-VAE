...
from torch_sparse import padded_index
from NonEuclideanStuff.misc.losses import *
...
if __name__ == '__main__':
    ...
    intra_loader = DataListLoader(dataset_train, batch_size=2, shuffle=True, num_workers=0, sampler=IntraSampler(dataset_train))
    interp_loader = DataListLoader(dataset_train, batch_size=2, shuffle=True, num_workers=0)
    rec_loader = DataListLoader(dataset_train, batch_size=config['train_batch_size'], shuffle=True, num_workers=0)
    ...
    for i in range(NUM_ITERS+1):
        ...
        if i%10 == 0:
            ...
            print("Iter n° {}: Loss: {:.2e} ({:.2e}, {:.2e}, {:.2e}, {:.2e})".format(i, avg_loss[0], avg_loss[1], avg_loss[3], avg_loss[4]))

        # Save checkpoint
        if i%100 == 0:
            if i <= 3000:
                save(model.state_dict(), 'logs/checkpoints/{}_{}_loss={:.2f}.dict'.format(config['model_params']['model_name'], config['data_params']['dataset']), loss)
            elif i <= 4000:
                save(model.state_dict(), 'logs/checkpoints/{}_{}_loss={:.2f}_EUCL.dict'.format(config['model_params']['model_name'], config['data_params']['dataset']), loss)
            else:
                save(model.state_dict(), 'logs/checkpoints/{}_{}_loss={:.2f}_GEOD.dict'.format(config['model_params']['model_name'], config['data_params']['dataset']), loss)    
